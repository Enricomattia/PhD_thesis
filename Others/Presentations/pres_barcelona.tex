...
\begin{frame}{Model: acts}
	...
	\begin{columns}
		...
		\begin{column}{0.55\textwidth}  % Adjusted right column to 0.55\textwidth
			\begin{itemize}
				\item \textcolor{blue}{outcome} set (net financial return);
			\end{itemize}
		\end{column}
	\end{columns}
...
\begin{frame}[noframenumbering,plain]{Axioms: Strategic Rationality for Best Likelihood}\label{srblapp}
	...
	\begin{axiom}\label{ax:appsrbl}
		...
		(\textbf{Strategic Rationality for Best Likelihood}). For all contingent menus \( F \), menus \( M, M^{\prime} \), events \( S^{\prime} \) and likelihoods \( \ell_{S^{\prime}} \), if \( \ell_{M,F} = \ell_{S^{\prime}} \), \( \left( M \cup M^{\prime} \right)_{\ell_{S^{\prime}}} \cap \left( M \cup M^{\prime} \right)_{\ell^{*}_{S^{\prime}}} \neq \emptyset \) for some \( \ell_{S^{\prime}}^{*} \), then
