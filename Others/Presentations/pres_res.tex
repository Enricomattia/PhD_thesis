...
\begin{frame}{Illustrative Example}
	...
	Investing is optimal only in the \textit{Good} state.
...
\begin{frame}[noframenumbering]{Illustrative Example}
	...
	She likes believing the state is good:

	\vfill

	\begin{wideitemize}
		\item under \( \textcolor{RoyalBlue}{\overline{p}} \): overweights the positive signal and invests;
		\item under \( \textcolor{red}{\underline{p}} \): suffers from the bad news.
	\end{wideitemize}
...
\begin{frame}[noframenumbering]{Illustrative Example}
	...
	She might also prefer not to check because she expects the bad signal.
...
\begin{frame}[noframenumbering]{Illustrative Example}
	...
	\textbf{Idea}: commit not to invest, e.g. by delegating to a financial advisor or algorithm.
...
\begin{frame}{Utility over contingent menus}\label{fullmodel}
	...
	The positive number \( \alpha_{\ell} \) captures the \textit{strength of motivated reasoning}.
